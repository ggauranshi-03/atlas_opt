...
class MuonSAMFrob(torch.optim.Optimizer):
    """
    SAM+Muon with Frobenius normalization: two-pass SAM with a layerwise Frobenius inner step for matrix
    ...
    def __init__(self, base_optimizer, rho=0.0015, rho_vector=0.01):
        ...
        super(MuonSAMFrob, self).__init__(self.param_groups, self.defaults)
    ...
    def first_step(self, zero_grad=False):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if p.ndim >= 2:
                    # 2D (Matrices): Frobenius Geometry
                    # We treat the entire 2D matrix as a flat vector for the Frobenius norm calculation
                    g_2d = p.grad.float()
                    frob_norm = g_2d.norm(p='fro') + 1e-12
                    
                    # Perturbation: w_adv = w_0 + rho * (G_t / ||G_t||_F)
                    P_t = g_2d / frob_norm
