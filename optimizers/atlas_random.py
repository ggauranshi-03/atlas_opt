...
class AtlasOptimizerRandom(torch.optim.Optimizer):
    """
    Atlas Optimizer (Random Matrix): Uses a random matrix (divided by its Frobenius norm)
    for the perturbation direction, instead of Newton-Schulz orthogonalized gradients.
    Actual parameter updates still use Newton-Schulz orthogonalized gradients.
    """
    def __init__(self, model_or_params, lr=0.02, weight_decay=0.01, rho=0.05, rho_vector=0.05, momentum=0.9665, nesterov=True, ns_steps=5, adam_lr=3e-4):
    ...
    def step(self, closure=None):
        if closure is None: raise RuntimeError("AtlasOptimizerRandom requires a closure")
            ...
            for group in self.param_groups:
                for p in group["params"]:
                    ...
                    if group.get("use_muon", False):
                        self.state[p]["is_initialized"] = True
                    else:
            ...
            for p in group["params"]:
                ...
                if use_muon:
                    if "is_initialized" in state:
                        ...
                        P_rand = torch.randn_like(p.data)
                        p_norm = P_rand.norm(p="fro") + 1e-8
                        p.data.add_(P_rand.to(p.dtype), alpha=rho / p_norm)
                else:
                    if "v_stale" in state:
                        stale = state["v_stale"]
                        state["old_v"] = p.data.clone()
                        s_norm = stale.norm(p="fro") + 1e-8
