...
class AtlasOptimizerRaw(torch.optim.Optimizer):
    """
    Atlas Optimizer (Raw Gradient): Uses the raw gradient divided by its Frobenius norm
    for the perturbation direction, instead of Newton-Schulz orthogonalization.
    ...
    def step(self, closure=None):
        if closure is None: raise RuntimeError("AtlasOptimizerRaw requires a closure")
            ...
            for group in self.param_groups:
                for p in group["params"]:
                    ...
                    if group.get("use_muon", False):
                        self.state[p]["g_stale"] = p.grad.float().clone()
                    else:
        ...
        for group in self.param_groups:
            ...
            stale_key = "g_stale" if use_muon else "v_stale"
