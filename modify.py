...
with open("atlas_optimized_sam.py", "r") as f:
    content = f.read()

# 1. Replace HybridSAMOptimizer block
new_optimizer = """
...
pattern = r"class HybridSAMOptimizer\(torch\.optim\.Optimizer\):.*?return loss\n"
content = re.sub(pattern, new_optimizer, content, flags=re.DOTALL)

# 2. Replace is_sam_like
content = content.replace("isinstance(opt, HybridSAMOptimizer)", "isinstance(opt, OptimizedHybridSAM)")

# 3. Replace atlas block in make_optimizer
