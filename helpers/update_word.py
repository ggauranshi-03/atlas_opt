...
def main():
    doc_path = r"G:\Gauranshi\IIITD-Research\Atlas_opt\Opt-exp-res\ATLAS OPTIMIZER EXPERIMENTS_060926 (1).docx"
    doc = Document(doc_path)
    
    doc.add_page_break()
    doc.add_heading("FINAL UPDATED RESULTS (Including Muon-SAM Frob & Stale Variants)", level=1)
    
    runs = [
        ('Experiment 1: CIFAR-10 Image Classification', 'final_logs/run1_cifar10_logs.csv', 'final_logs/run1_cifar10_plot.png', None),
        ('Experiment 2: nanoGPT Continual Pre-Training', 'final_logs/run2_nanogpt_logs.csv', 'final_logs/run2_nanogpt_plot.png', 'final_logs/run2_nanogpt_perplexity_plot.png'),
        ('Experiment 3: Pythia-70M Continual Pre-Training', 'final_logs/run3_pythia_logs.csv', 'final_logs/run3_pythia_plot.png', 'final_logs/run3_pythia_perplexity_plot.png')
    ]
    
    for title, csv_path, plot_path, ppl_plot_path in runs:
        ...
        if ppl_plot_path and os.path.exists(ppl_plot_path):
            doc.add_picture(ppl_plot_path, width=Inches(4.5))
